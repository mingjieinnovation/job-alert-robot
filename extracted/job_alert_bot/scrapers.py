...
def _passes_title_filter(title: str) -> bool:
    ...
    if config.TITLE_EXCLUDE:
        if any(kw.lower() in t for kw in config.TITLE_EXCLUDE):
            return False
    # Exclude contract jobs
    if hasattr(config, 'CONTRACT_KEYWORDS'):
        if any(kw.lower() in t for kw in config.CONTRACT_KEYWORDS):
            return False
    return True


def _is_contract_job(text: str) -> bool:
    """Check if the job text indicates it's a contract role."""
    t = f" {text.lower()} "
    if hasattr(config, 'CONTRACT_KEYWORDS'):
    ...
    if re.search(r'\b(contract|contractor)\b', t):
        return True
    return False


def _has_ai_mention(text: str) -> bool:
    """检查文本中是否至少提到一次 AI 相关内容"""
    t = text.lower()
    ai_patterns = [
        ...
        "computer vision", "agentic", "gpt", "chatgpt",
    ]
    # 在文本前后加空格方便匹配 " ai "
    padded = f" {t} "
    return any(p in padded for p in ai_patterns)


def _exceeds_max_experience(text: str, max_years: int = 5) -> bool:
    """检查描述是否要求超过 max_years 年经验"""
    t = text.lower()
    # 检查配置中的排除关键词
    for kw in config.DESCRIPTION_EXCLUDE_KEYWORDS:
        if kw.lower() in t:
            return True
    # 检查 "X+ years" 或 "X years" 模式
    years_match = re.findall(r'(\d+)\+?\s*years?', t)
...
def _score_job(job: Job) -> Job:
    ...
    salary_matches = re.findall(r'£\s*([\d,]+)\s*(?:k|K)?', salary_text)
    for m in salary_matches:
        val = int(m.replace(',', ''))
        if val < 1000:
            val *= 1000
        if 20000 <= val < config.MIN_SALARY:
            job.match_score = -99
            job.match_tags = ["❌薪资<£45k"]
            job.experience_ok = False
            return job

    # 检查是否是 Contract 职位（硬性排除）
    if _is_contract_job(text):
        job.match_score = -99
        job.match_tags = ["❌Contract"]
        job.experience_ok = False
        return job
...
def fetch_all_jobs() -> List[Job]:
    ...
    # 过滤掉不合格的（无AI提及 或 经验>5年）
    before_count = len(all_jobs)
    all_jobs = [j for j in all_jobs if j.match_score > -99]
    filtered_count = before_count - len(all_jobs)
    if filtered_count > 0:
        logger.info(f"\n🚫 硬性过滤掉: {filtered_count} 个（无AI提及或经验>5年）")
